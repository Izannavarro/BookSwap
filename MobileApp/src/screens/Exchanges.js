...
export default Exchanges = () => {
  ...
  useEffect(() => {
    if (
      ...
      setFormVisible(true);
      setHasShownModal(true); // Marcar que ya lo mostramos
      navigation.setParams({ showCreateModal: false, selectedUser: null });
    }
  }, [route.params, loadingUserBooks, loadingUsersInfo, hasShownModal]);
  ...
  const updateExchangeStatus = async (exchangeId, ownerId, newStatus) => {
    try {
      ...
      Alert.alert('Error', 'Could not update status or remove the book.');
    }
  };
